...
from neat_src1.ann import getLayer, getNodeOrder, act


class Ind():
  """Individual class: genes, network, and fitness
  """ 
  def __init__(self, conn, node, wGrad=[]):
  ...
  def predict(self, x): 
    """Takes in observations and returns an action
    Args:
      x - (array-like) [12,] of observations
    Outputs:
      h - (np_array) [3x1] specifying action
    """
    h = act(self.wMat, self.aVec, nInput=12, nOutput=3, inPattern=x)
    return h.tolist()[0]
  ...
  def createChild(self, p, innov, gen=0, mate=None):
    ...
    if mate is not None:
      ...
      child = Ind(self.conn, self.node, self.wGrad)
...
# -- Canonical NEAT recombination operators ------------------------------ -- #
  ...
  def mutate(self,p,innov=None,gen=None):
    ...
    if (np.random.rand() < p['prob_mutAct']):
      print("""
                ...
                =====================
            """)
      nodeidx = np.random.randint(len(nodeG[2,:]))
      nodeG[2,nodeidx] = p['ann_actRange'][np.random.randint(len(p['ann_actRange']))]
  ...
  def mutAddNode(self, connG, nodeG, innov, gen, p):
    ...
    connTo[0] = newConnId
    connTo[2] = newNodeId
    connTo[3] = 1 - 0.1 + 0.2*np.random.rand() # weight set to 1
      
    connFrom    = connG[:,connSplit].copy()
    connFrom[0] = newConnId + 1
    connFrom[1] = newNodeId
    connFrom[3] = connG[3,connSplit] + 0.2*np.random.rand() # weight set to previous weight value   
